...
PORT = int(os.getenv("PORT", 10000))  # Render/Heroku default
...
def should_migrate_csv(csv_file, db_file):
    """Chỉ migrate CSV sang SQLite nếu CSV mới hơn DB hoặc chưa có DB"""
    if not os.path.exists(db_file):
        return True
    return os.path.getmtime(csv_file) > os.path.getmtime(db_file)
...
def csv_to_sqlite(csv_file, db_file, table_name="rep"):
    """Chuyển đổi file CSV sang SQLite, tối ưu nhập liệu"""
...
def refresh_data():
    """Làm mới dữ liệu từ DB và cache, chỉ reload khi quá hạn TTL"""
    now = time.time()
    if now - state.last_refresh > CACHE_TTL:
        state.data = load_data_from_sqlite(SQLITE_FILE_PATH, "rep")
        state.last_refresh = now
        state.last_cache_refresh = 0  # Force cache refresh
...
async def send_message(update, message_func, text, **kwargs):
    """Gửi tin nhắn và lưu message_id để xóa về sau"""
    try:
...
async def delete_messages(update, context):
    """Xóa các tin nhắn cũ đã lưu của user"""
    ...
    try:
        ...
        keyboard = [
            [InlineKeyboardButton(
                key_group[0],
                callback_data=f"key:{state.get_id(key_group[0])}::"
            )]
...
async def handle_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if level == "key":
            rep1s = set()
            for r1_set in state.rep1_groups[norm_key].values():
                rep1s.update(r1_set)
            if rep1s:
                keyboard = [
            ...
            norm_rep1 = norm(rep1)
            rep2s = set()
            for rep2_set in state.rep2_groups[norm_key][norm_rep1].values():
...
if __name__ == '__main__':
    # Chỉ migrate khi cần
    if os.path.exists(CSV_FILE_PATH) and should_migrate_csv(CSV_FILE_PATH, SQLITE_FILE_PATH):
        try:
